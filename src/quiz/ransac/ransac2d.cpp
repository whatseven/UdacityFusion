...
std::unordered_set<int> Ransac(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
  ...
  for (size_t i = 0; i < maxIterations; i++) {
    ...
    size_t pointIndex1 = static_cast<size_t>(ud(mt));
    size_t pointIndex2 = static_cast<size_t>(ud(mt));
    while (pointIndex1==pointIndex2)
      pointIndex2 = static_cast<size_t>(ud(mt));

    float A = cloud->points[pointIndex1].y - cloud->points[pointIndex2].y;
    float B = cloud->points[pointIndex2].x - cloud->points[pointIndex1].x;
    float C = cloud->points[pointIndex1].x * cloud->points[pointIndex2].y -
              cloud->points[pointIndex2].x * cloud->points[pointIndex1].y;
    float sqrta2b2 = sqrtf(A * A + B * B);

    size_t inliers = 0;
    size_t itemIndex = 0;
    for (auto pointItem : cloud->points) {
      if (abs(A * pointItem.x + B * pointItem.y + C) / sqrta2b2 < distanceTol) {
        inliers++;
...
int main() {
  ...
  // function
  std::unordered_set<int> inliers = Ransac(cloud, 100, 0.2f);
